...
CONF_TRUMA_EEZY_ID = "truma_eezy_id"
...
truma_eezy_ns = cg.esphome_ns.namespace("truma_eezy")
StatusFrameHeater = truma_eezy_ns.struct("StatusFrameHeater")
StatusFrameHeaterConstPtr = StatusFrameHeater.operator("ptr").operator("const")
TrumaEezyApp = truma_eezy_ns.class_(
    "TrumaEezyApp", cg.PollingComponent, uart.UARTDevice
)
TrumaEezyAppHeaterMessageTrigger = truma_eezy_ns.class_(
    "TrumaEezyAppHeaterMessageTrigger",
    automation.Trigger.template(StatusFrameHeaterConstPtr),
)

# `LIN_CHECKSUM` is a enum class and not a namespace but it works.
LIN_CHECKSUM_dummy_ns = truma_eezy_ns.namespace("LIN_CHECKSUM")
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(TrumaEezyApp),
            ...
            cv.Optional(CONF_ON_HEATER_MESSAGE): automation.validate_automation(
                {
                    cv.GenerateID(CONF_TRIGGER_ID): cv.declare_id(TrumaEezyAppHeaterMessageTrigger),
                }
            ),
        }
    )
...
FINAL_VALIDATE_SCHEMA = cv.All(
    final_validate_device_schema(
        "truma_eezy", baud_rate=9600, require_tx=True, require_rx=True, stop_bits=2, data_bits=8, parity="NONE", require_hardware_uart=True),
)
...
HeaterRoomTempAction = truma_eezy_ns.class_(
    "HeaterRoomTempAction", automation.Action)
HeaterWaterTempAction = truma_eezy_ns.class_(
    "HeaterWaterTempAction", automation.Action)
HeaterWaterTempEnumAction = truma_eezy_ns.class_(
    "HeaterWaterTempEnumAction", automation.Action)
HeaterElecPowerLevelAction = truma_eezy_ns.class_(
    "HeaterElecPowerLevelAction", automation.Action)
HeaterEnergyMixAction = truma_eezy_ns.class_(
    "HeaterEnergyMixAction", automation.Action)
AirconManualTempAction = truma_eezy_ns.class_(
    "AirconManualTempAction", automation.Action)
TimerDisableAction = truma_eezy_ns.class_(
    "TimerDisableAction", automation.Action)
TimerActivateAction = truma_eezy_ns.class_(
    "TimerActivateAction", automation.Action)
WriteTimeAction = truma_eezy_ns.class_("WriteTimeAction", automation.Action)

# `EnergyMix` is a enum class and not a namespace but it works.
EnergyMix_dummy_ns = truma_eezy_ns.namespace("EnergyMix")
...
ElectricPowerLevel_dummy_ns = truma_eezy_ns.namespace("ElectricPowerLevel")
...
HeatingMode_dummy_ns = truma_eezy_ns.namespace("HeatingMode")
...
TargetTemp_dummy_ns = truma_eezy_ns.namespace("TargetTemp")
...
@automation.register_action(
    "truma_eezy.heater.set_target_room_temperature",
    HeaterRoomTempAction,
    automation.maybe_conf(
        CONF_TEMPERATURE,
        {
            cv.GenerateID(): cv.use_id(TrumaEezyApp),
...
async def truma_eezy_heater_set_target_room_temperature_to_code(config, action_id, template_arg, args):
...
@automation.register_action(
    "truma_eezy.heater.set_target_water_temperature",
...
async def truma_eezy_heater_set_target_water_temperature_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
@automation.register_action(
    "truma_eezy.heater.set_target_water_temperature_enum",
...
async def truma_eezy_heater_set_target_water_temperature_enum_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
@automation.register_action(
    "truma_eezy.heater.set_electric_power_level",
...
async def truma_eezy_heater_set_electric_power_level_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
@automation.register_action(
    "truma_eezy.heater.set_energy_mix",
...
async def truma_eezy_heater_set_energy_mix_level_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
@automation.register_action(
    "truma_eezy.aircon.manual.set_target_temperature",
...
async def truma_eezy_aircon_manual_set_target_temperature_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def truma_eezy_timer_disable_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    return var
...
async def truma_eezy_timer_activate_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
...
async def truma_eezy_clock_set_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    await cg.register_parented(var, config[CONF_ID])
    return var
