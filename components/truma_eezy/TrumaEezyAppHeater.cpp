...
static const char *const TAG = "truma_eezy.TrumaEezyAppHeater";

StatusFrameHeaterResponse *TrumaEezyAppHeater::update_prepare() {
...
void TrumaEezyAppHeater::create_update_data(StatusFrame *response, u_int8_t *response_len,
...
void TrumaEezyAppHeater::dump_data() const {}

bool TrumaEezyAppHeater::can_update() {
...
bool TrumaEezyAppHeater::action_heater_room(u_int8_t temperature, HeatingMode mode) {
  if (!this->can_update()) {
...
bool TrumaEezyAppHeater::action_heater_water(u_int8_t temperature) {
  if (!this->can_update()) {
    ESP_LOGW(TAG, "Cannot update Truma.");
    return false;
  }
...
bool TrumaEezyAppHeater::action_heater_water(TargetTemp temperature) {
  if (!this->can_update()) {
    ESP_LOGW(TAG, "Cannot update Truma.");
    return false;
  }
...
bool TrumaEezyAppHeater::action_heater_electric_power_level(u_int16_t value) {
  if (!this->can_update()) {
    ESP_LOGW(TAG, "Cannot update Truma.");
    return false;
  }
...
bool TrumaEezyAppHeater::action_heater_energy_mix(EnergyMix energy_mix, ElectricPowerLevel el_power_level) {
  if (!this->can_update()) {
    ESP_LOGW(TAG, "Cannot update Truma.");
    return false;
  }
