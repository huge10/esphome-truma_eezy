...
static const char *const TAG = "truma_eezy.TrumaEezyAppClock";

void TrumaEezyAppClock::dump_data() const {
  ESP_LOGD(TAG, "StatusFrameClock %02d:%02d:%02d", this->data_.clock_hour, this->data_.clock_minute,
           this->data_.clock_second);
}

#ifdef USE_TIME
bool TrumaEezyAppClock::action_write_time() {
  if (!this->can_update()) {
...
void TrumaEezyAppClock::create_update_data(StatusFrame *response, u_int8_t *response_len, u_int8_t command_counter) {
  if (this->parent_->get_time() != nullptr) {
