...
static const char *const TAG = "truma_eezy.TrumaEezyApp";

TrumaEezyApp::TrumaEezyApp() {
...
const std::array<uint8_t, 4> TrumaEezyApp::lin_identifier() {
...
void TrumaEezyApp::lin_heartbeat() { this->device_registered_ = micros(); }

void TrumaEezyApp::lin_reset_device() {
...
bool TrumaEezyApp::answer_lin_order_(const u_int8_t pid) {
  // Alive message
...
bool TrumaEezyApp::lin_read_field_by_identifier_(u_int8_t identifier, std::array<u_int8_t, 5> *response) {
...
const u_int8_t *TrumaEezyApp::lin_multiframe_recieved(const u_int8_t *message, const u_int8_t message_len,
                                                         u_int8_t *return_len) {
...
bool TrumaEezyApp::has_update_to_submit_() {
